...
export default function PersonalColoring() {
  ...
  const season = palette?.season_name ?? "Ainda não definida";

  const description =
    palette?.description ?? "Sua análise de coloração pessoal ainda não foi registrada pela estrategista.";
    ...
    <div className="p-4 md:p-8 max-w-4xl">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl md:text-4xl font-display font-light">Minha Coloração</h1>
        </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="card-luxury p-6 md:p-8">
        <span className="text-xs text-gold uppercase tracking-wider font-medium">Estação de cor</span>

        <h2 className="font-display text-3xl md:text-4xl mt-2 mb-4">{season}</h2>
