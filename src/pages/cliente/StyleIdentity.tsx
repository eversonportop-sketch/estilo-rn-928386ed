...
export default function ClientStyleIdentity() {
  ...
  useEffect(() => {
    const loadStyles = async () => {
      ...
      if (data) {
        const mapped = data.map((item: any) => item.style_profiles).filter(Boolean);
  ...
  const predominant =
    styles.length > 0
      ? defaultStyles.find((s) => s.name === styles[0].name) || defaultStyles[0]
      : defaultStyles.find((s) => s.name === "Sofisticada") || defaultStyles[0];

  const secondary =
    styles.length > 1
      ? defaultStyles.find((s) => s.name === styles[1].name) || defaultStyles[1]
      : defaultStyles.find((s) => s.name === "Contemporânea") || defaultStyles[1];
    ...
    <div className="p-4 md:p-8 max-w-4xl">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-muted-foreground text-xs md:text-sm mb-8 md:mb-10">
          Sua personalidade de moda definida pela análise de estilo
        </p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        >
          ...
          <div className="flex flex-wrap gap-2">
            {predominant.tags.map((tag: string) => (
              <span key={tag} className="text-[10px] px-2 py-1 rounded-full bg-gold/10 text-gold border border-gold/20">
                {tag}
              </span>
            ))}
          </div>
        ...
        <motion.div
          ...
          className="card-luxury p-6 md:p-8 opacity-80"
        >
          ...
          <div className="flex flex-wrap gap-2">
            {secondary.tags.map((tag: string) => (
              <span
                key={tag}
                className="text-[10px] px-2 py-1 rounded-full bg-muted/50 text-muted-foreground border border-border"
              >
                {tag}
              </span>
            ))}
