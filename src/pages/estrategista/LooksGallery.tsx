...
export default function LooksGallery() {
  ...
  return (
    <div className="p-8 lg:p-12 max-w-6xl">
          ...
          <motion.div key={look.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className="card-luxury overflow-hidden group">
            ...
            <div className="p-6">
              ...
              <span className={`text-[10px] px-2 py-0.5 rounded-full ${look.criadoPor === "estrategista" ? "bg-gold/10 text-gold-dark" : "bg-muted text-muted-foreground"}`}>
                {look.criadoPor === "estrategista" ? "Criado pela estrategista" : "Criado pela cliente"}
              </span>
            </div>
          </motion.div>
        ))}
      ...
      {showForm && <LookForm onSave={handleSave} onClose={() => { setShowForm(false); setEditingLook(undefined); }} pecas={pecas} initial={editingLook} criadoPor="estrategista" />}
      {showSmartGen && <SmartLookGenerator onSave={handleSave} onClose={() => setShowSmartGen(false)} generateSmartLook={generateSmartLook} criadoPor="estrategista" />}
    </div>
  );
}
