...
import { Upload, Plus, Image, Trash2, Tag } from "lucide-react";

interface PhotoRef {
  id: string;
  url: string;
  title: string;
  category: "cliente" | "referencia" | "analise";
  notes: string;
}
...
const mockPhotos: PhotoRef[] = [
  { id: "1", url: "https://images.unsplash.com/photo-1469334031218-e382a71b716b?w=400", title: "Referência Look Trabalho", category: "referencia", notes: "Estilo corporate elegante" },
  { id: "2", url: "https://images.unsplash.com/photo-1485968579580-b6d095142e6e?w=400", title: "Análise de Cores", category: "analise", notes: "Paleta inverno profundo identificada" },
  { id: "3", url: "https://images.unsplash.com/photo-1483985988355-763728e1935b?w=400", title: "Referência Look Evento", category: "referencia", notes: "Inspiração look noite" },
];

export default function StrategistPhotos() {
  const [photos, setPhotos] = useState<PhotoRef[]>(mockPhotos);
  const [filter, setFilter] = useState<string>("todos");
  const [showForm, setShowForm] = useState(false);
  const [newTitle, setNewTitle] = useState("");
  const [newCategory, setNewCategory] = useState<PhotoRef["category"]>("referencia");
    ...
    <div className="p-4 md:p-8 max-w-6xl">
      ...
      {showForm && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="card-luxury p-4 md:p-6 mb-6">
          <h3 className="font-display text-base md:text-lg mb-4">Adicionar Foto ou Referência</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <select
                value={newCategory}
                onChange={e => setNewCategory(e.target.value as PhotoRef["category"])}
                className="w-full px-4 py-2.5 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-1 focus:ring-gold/40"
              >
            ...
            <div className="md:col-span-2">
              <div className="border-2 border-dashed border-border/50 rounded-xl p-6 text-center hover:border-gold/30 transition-colors cursor-pointer">
          ...
          <div className="flex gap-3">
            <button onClick={addPhoto} className="px-4 py-2.5 rounded-xl gold-gradient text-primary-foreground text-sm hover:opacity-90 transition-opacity">
              Salvar
            </button>
            <button onClick={() => setShowForm(false)} className="px-4 py-2.5 rounded-xl border border-border text-sm hover:bg-muted/50 transition-colors">
              Cancelar
            </button>
          </div>
        </motion.div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="text-sm">Adicione fotos e referências visuais.</p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="p-3 md:p-4">
                <div className="flex items-start justify-between gap-2 mb-1">
                  <h3 className="font-display text-sm md:text-base">{photo.title}</h3>
                </div>
                <div className="flex items-center gap-1 mb-2">
                  <Tag className="w-3 h-3 text-gold" />
                  <span className="text-[10px] text-gold">{categoryLabel(photo.category)}</span>
                </div>
